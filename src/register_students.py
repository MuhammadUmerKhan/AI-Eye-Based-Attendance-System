...
def register_studentss(db, student_id: str, name: str, department: str, image_input, faiss_index: FaissIndex) -> Tuple[bool, str]:
    """
    Register a student by saving their ID, name, department, and eye region embedding to the database,
    and update the FAISS index.
    Accepts a Database instance, image input (file path or file-like object), and a FaissIndex instance.
    Returns success status and message.
    """
    try:
        logger.debug(f"Registering student {student_id}, name: {name}")
        ...
        if not success:
            logger.error({"student_id": student_id, "message": message})
            return False, message
        
        embedding = embedding.astype(np.float32)
        faiss_index.update_index(embedding, student_id, name)
        logger.info({"student_id": student_id, "message": f"{message} and FAISS index updated"})
